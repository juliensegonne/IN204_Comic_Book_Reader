...
FullScreenLabel::FullScreenLabel(Book& book, qreal zoom,QWidget *parent )
    : QLabel(parent), bookRef(book), currentPageIndex(-1), zoomFactor(zoom), x_zoom(0.0),y_zoom(0.0) {
...
void FullScreenLabel::wheelEvent(QWheelEvent *event) {
    ...
    if (rect().contains(event->position().toPoint())) {
        ...
        if (event->angleDelta().y() > 0) {
            zoomFactor = 1.05*zoomFactor;
        } else {
            zoomFactor = zoomFactor / 1.05;
        }
        // Appeler la fonction pour définir le facteur de zoom avec le nouveau facteur calculé
...
void FullScreenLabel::afficherPageCourante() {
    ...
    if (currentPageIndex < static_cast<int>(pages.size())) {
        ...
        int nouvelleLargeur = std::floor(maxImageSize.width() * zoomFactor + x_zoom);
        int nouvelleHauteur = std::floor(maxImageSize.height() * zoomFactor + y_zoom);
        maxImageSize.setWidth(nouvelleLargeur);
        maxImageSize.setHeight(nouvelleHauteur);
        for (const auto& image : images) {
            // Charger l'image
            QImage img(QString::fromStdString(image.ObtenirNomFichier()));
